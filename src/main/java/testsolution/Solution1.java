...
//        Arrays.sort(candidates);//所给数组可能无序，排序保证解按照非递减组合
        List<Integer> list = new ArrayList<Integer>();
        int total = 0;
        backtracking(candidates, target, 0, list,total);//给定target，start=0表示从数组第一个开始
        System.out.println(total);
        return result;//返回解的组合链表
    }

    public static void backtracking(int[] candidates, int target, int start, List<Integer> list,int total) {
        ...
        if (target < 0) return;//凑过头了
            ...
            for (int i = start; i < candidates.length; i++) {//循环试探每个数
                ...
                backtracking(candidates, target - candidates[i], i+1, list,total);
